...
def get_last_post(feed_url):
	feed_content = feedparser.parse(feed_url)
	post = []
	post.append(feed_content['items'][0]['title'])
	post.append(feed_content['items'][0]['link'])
	post.append(feed_content['items'][0]['published'])
	post.append(re.search('(?P<url>https?://[^\s"]+)', feed_content['items'][0]['summary_detail']['value']).group('url'))
	return post

def get_story_image(feed_url):
	post = get_last_post(feed_url)
